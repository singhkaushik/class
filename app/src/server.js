...
const hostUsersString = process.env.HOST_USERS || '[{"username": "liveclass", "password": "123456"}]';
...
// Room presenters
const roomPresentersString = process.env.PRESENTERS || '["Live Class Ias Academy"]';
...
const api_key_secret = process.env.API_KEY_SECRET || 'liveclass_default_secret';
...
app.post([apiBasePath + '/meeting'], (req, res) => {
    ...
    if (authorization != api_key_secret) {
        log.debug('Live Class get meeting - Unauthorized', {
            headers: headers,
            body: body,
        });
    ...
    log.debug('Live Class get meeting - Authorized', {
        headers: headers,
        body: body,
        meeting: meetingURL,
    });
...
function getMeetingURL(host) {
    return 'https' + (host.includes('localhost') ? '' : 's') + '://' + host + '/join/' + uuidV4();
}
